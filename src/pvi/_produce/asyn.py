...
"""
    ...
    def _produce_record(self, parameter: AsynParameter) -> Iterator[Record]:
        ...
        if parameter.access.needs_read_record():
            ...
            record = getattr(records, rtype)(name, **inp_fields)
            self.add_pvi_info(record, "r")
        ...
        if parameter.access.needs_write_record():
            ...
            if parameter.demand_auto_updates:
                ...
                self.add_pvi_info(record, "rw")
            else:
                self.add_pvi_info(record, "w")

            yield record

    def add_pvi_info(self, record: Record, access: str) -> None:
        """Add info field to record to populate PVI PV"""
        record.add_info(
            "Q:group",
            {
                "$(P)$(R):PVI": {
                    f"pvi.{record.name}.{access}": {
                        "+channel": "NAME",
                        "+type": "plain",
                    }
                }
